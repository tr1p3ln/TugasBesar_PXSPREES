        ...
        <div
            ...
            <div class="p-6 sm:p-8">
                <div class="mb-8 text-center">
                    <h2 class="text-3xl font-bold text-white">Edit Ruangan: {{ $room->name }}</h2>
                    <p class="text-gray-400 mt-2">Perbarui detail ruangan di bawah ini.</p>
                </div>
                ...
                {{-- PASTIKAN BARIS INI BENAR: action menyertakan $room->id --}}
                <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="name"
                            ...
                            placeholder="Contoh: Ruang PS-1 atau VIP Room A" value="{{ old('name', $room->name) }}"
                            required>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="console_type" id="console_type"
                                class="w-full px-4 py-3 rounded-lg bg-gray-700/50 text-white border-2 border-gray-600 focus:border-purple-500 focus:ring-purple-500 focus:outline-none transition"
                                required>
                                <option value="Playstation" @if (old('console_type', $room->console_type) == 'Playstation') selected @endif>
                                    Playstation</option>
                                <option value="Nintendo" @if (old('console_type', $room->console_type) == 'Nintendo') selected @endif>Nintendo
                                </option>
                                <option value="VIP" @if (old('console_type', $room->console_type) == 'VIP') selected @endif>VIP</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="price_per_hour" id="price_per_hour"
                                class="w-full px-4 py-3 rounded-lg bg-gray-700/50 text-white border-2 border-gray-600 focus:border-purple-500 focus:ring-purple-500 focus:outline-none transition"
                                placeholder="Contoh: 50000" value="{{ old('price_per_hour', $room->price_per_hour) }}"
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="w-full px-4 py-3 rounded-lg bg-gray-700/50 text-white border-2 border-gray-600 focus:border-purple-500 focus:ring-purple-500 focus:outline-none transition"
                            placeholder="Jelaskan fasilitas yang ada di ruangan ini..." required>{{ old('description', $room->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            class="w-full px-4 py-3 rounded-lg bg-gray-700/50 text-white border-2 border-gray-600 focus:border-purple-500 focus:ring-purple-500 focus:outline-none transition"
                            required>
                            <option value="Available" @if (old('status', $room->status) == 'Available') selected @endif>Available
                            </option>
                            <option value="Booked" @if (old('status', $room->status) == 'Booked') selected @endif>Booked</option>
                            <option value="Maintenance" @if (old('status', $room->status) == 'Maintenance') selected @endif>Maintenance
                            </option>
                            <option value="Disabled" @if (old('status', $room->status) == 'Disabled') selected @endif>Disabled</option>
                        </select>
                    </div>
