<x-layouts.admin>
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto bg-gray-800 rounded-lg shadow-lg p-6">
            <h2 class="text-2xl font-bold text-white mb-6">Edit Ruangan</h2>
            ...
            <div>
                ...
                <select name="console_type" id="console_type"
                    ...
                    <option value="{{ $type }}" {{ old('console_type', $room->console_type) == $type ? 'selected' : '' }}>
                        {{ $type }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="number" name="price_per_hour" id="price_per_hour"
                    class="w-full px-4 py-2 rounded bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none"
                    value="{{ old('price_per_hour', $room->price_per_hour) }}" min="0" step="1000" required>
            </div>

            <!-- <div>
                <label for="description" class="block text-white mb-2">Deskripsi</label>
                <textarea name="description" id="description" rows="3" class="w-full px-4 py-2 rounded bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none" required>{{ old('description', $room->description) }}</textarea>
            </div> -->
            ...
            <div>
                ...
                @if($room->image)
                <div class="mb-2">
                    <img src="{{ asset('storage/' . $room->image) }}" alt="Current Image" class="w-32 h-32 object-cover rounded">
                </div>
                @endif
                <input type="file" name="image" id="image"
                    class="w-full px-4 py-2 rounded bg-gray-700 text-white border border-gray-600 focus:border-blue-500 focus:outline-none"
                    accept="image/*">
                <p class="text-gray-400 text-sm mt-1">Biarkan kosong jika tidak ingin mengubah gambar. Format yang didukung: JPG, JPEG, PNG, GIF. Maksimal 2MB</p>
            </div>
