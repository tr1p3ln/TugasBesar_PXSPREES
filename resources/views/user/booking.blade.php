                ...
                <div class="container mx-auto px-4">
                    ...
                    <div id="booking-form" class="mt-12 max-w-2xl mx-auto bg-gray-900 p-8 rounded-xl ">
                        ...
                        <form id="booking-data" action="{{ route('user.booking.store') }}" method="POST">
                            ...
                            <input type="hidden" id="selected-time-slots-string"
                                name="selected_slots_string_for_debug">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div class="mb-4 md:col-span-2">
                                    <label for="voucher-code" class="block mb-2 font-medium">Kode Voucher
                                        (Opsional)</label>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                async function fetchRoomDataAndAvailability() {
                    ...
                    try {
                        const response = await fetch(
                            `/booking/check-availability?console_type=${selectedConsoleType}&date=${selectedDate}`, {
                                method: 'GET',
                                headers: {
                        ...
                        if (!response.ok) {
                            ...
                            roomSelect.innerHTML =
                                '<option value="" disabled selected>Error mengambil data</option>';
                            clearTimeSlots();
                            return;
                        }
                ...
                function populateRoomSelect(roomsData) {
                    ...
                    if (roomsData && roomsData.length > 0) {
                        ...
                        roomsData.forEach(room => {
                            ...
                            option.textContent =
                                `${room.name} (Rp ${parseFloat(room.price_per_hour).toLocaleString('id-ID')}/jam)`;
                ...
                function clearTimeSlots() {
                    timeSlotsContainer.innerHTML =
                        '<p class="text-gray-500 col-span-2 text-center">Pilih ruangan untuk melihat jam tersedia.</p>';
                    selectedTimeRanges = [];
                    updateBookingSummary();
                }
                ...
                function generateAndDisplayTimeSlots(bookedTimeRanges = []) {
                        ...
                        timeSlotDiv.addEventListener('click', function() {
                            ...
                            if (this.classList.contains('selected')) {
                                ...
                                if (selectedTimeRanges.length > 0) {
                                    ...
                                    const tempSortedSelected = [...selectedTimeRanges, clickedTimeRange].sort((
                                    ...
                                    if (!sequential) {
                                        ...
                                        const lastSelectedEnd = selectedTimeRanges[selectedTimeRanges.length -
                                            1].split('-')[1];
                            ...
                            selectedTimeSlotsStringInput.value = selectedTimeRanges.join(
                            ','); // Untuk debug atau referensi
                            updateBookingSummary();
                        });
                    ...
                    if (timeSlotsContainer.innerHTML === '') {
                        timeSlotsContainer.innerHTML =
                            '<p class="text-gray-500 col-span-2 text-center">Tidak ada jam tersedia untuk kombinasi ini.</p>';
                    }
                }
                ...
                serviceCards.forEach(card => {
                    ...
                    .toLowerCase(); // playstation, nintendo, vip
                        const serviceName = this.querySelector('h3').textContent;

                        consoleTypeInput.value = dbConsoleTypeMap[serviceKey] ||
                        ''; // Set hidden input console_type
                        ...
                        roomSelect.innerHTML =
                            '<option value="" disabled selected>Memuat ruangan...</option>';
                        clearTimeSlots();
                ...
                document.getElementById('booking-data').addEventListener('submit', async function(e) {
                    ...
                    if (selectedTimeRanges.length > 1) {
                        ...
                        for (let i = 0; i < selectedTimeRanges.length - 1; i++) {
                            if (selectedTimeRanges[i].split('-')[1] !== selectedTimeRanges[i + 1].split(
                                '-')[0]) {
                                sequential = false;
                                break;
                            }
                    ...
                    const firstSlotStartTime = selectedTimeRanges[0].split('-')[
                    0]; // Ambil jam mulai dari slot pertama
                    const finalStartTime =
                    `${bookingDate} ${firstSlotStartTime}:00`; // Format YYYY-MM-DD HH:MM:SS
                    ...
                    formData.set('total_price', parseFloat(document.getElementById('total-price')
                        .textContent.replace(/\./g, '').replace(/,/g, '.'))); // Pastikan format angka benar
                    ...
                    try {
                        const response = await fetch(
                        "{{ route('user.booking.store') }}", { // Menggunakan route Laravel
                            method: 'POST',
                            headers: {
                        ...
                        if (response.ok) {
                            ...
                            if (responseData.errors) {
                                ...
                                const errors = Object.values(responseData.errors).map(errArray => errArray
                                    .join('\n')).join('\n');
                                errorMessage += `\n\nDetail:\n${errors}`;
                            }
                            alert(errorMessage);
